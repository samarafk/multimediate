...
```{r check_multimediate, include = FALSE}
# Check if 'multimediate' is installed
if (!requireNamespace("multimediate", quietly = TRUE)) {
  stop(
    "This vignette requires the 'multimediate' package.\n",
...
We provide a short toy example that allows interested readers to reproduce the analysis from @RuizHernandez2017 using the `multimediate` package which implements counterfactual multiple mediation analysis in the setting of additive survival.


We first download and install the package:


```{r install_multimediate, eval = FALSE}
# Install from CRAN
install.packages("multimediate")
```


Now, we load the needed packages. We can verify that the package correctly uploaded by running  `packageVersion("multimediate")`. Which should return `[1] ‘0.1.6’`. 
...
The toy dataset provided in this vignette is a subset of the study population used in the manuscript by @RuizHernandez2017. The original dataset included imputation of missing completely at random urine cadmium values and blood lead values below the detection limit (for a deeper understanding of the imputation, see @RuizHernandez2017. Here we provided the subset of actually measured  (i.e. unimputed)  urine cadmium values. Also we extended the followup for mortality data collection to 10 years. The toy dataset can be downloaded from [here](https://raw.githubusercontent.com/samarafk/multimediate/refs/heads/master/vignettes/251003_Nhanes_Multimediate.csv).
