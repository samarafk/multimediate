...
multimediate = function(lmodel.m,
                        correlated = FALSE,
                        model.y,
                        treat,
                        treat.value = 1,
  ...
  if (inherits(model.y, "aalen")) {
      ...
      if (!inherits(lmodel.m[[nm]], "lm")) {
        for (c in 1:dim(lmodel.m[[nm]]$model)[2]) {
          if (!is.factor(lmodel.m[[nm]]$model[, c])) {
            stop(
              "covariate ",
              names(lmodel.m[[nm]]$model)[c],
              " in mediators models must be discretize and/or factorize."
            )
          }
        }
    ...
    for (nm in 1:NM) {
      MModel[[nm]] = rmvnorm(J,
                             mean = c(coef(lmodel.m[[nm]]), lmodel.m[[nm]]$zeta),
                             sigma = vcov(lmodel.m[[nm]]))
    ...
    YModel = rmvnorm(J,
                     mean = c(coef(model.y), model.y$zeta),
                     sigma = vcov(model.y))
    ...
    if (isTRUE(verbose))
      pb <- txtProgressBar(
        ...
        max = NM,
        style = 3,
        title = "Simulation of counterfactuals mediators"
      )
    ...
    for (nm in 1:NM) {
      ...
      if (inherits(lmodel.m[[nm]], "polr")) {
        ...
        PredictM1[, , nm] <- PredictM1b[, , nm] <- array(muM1, dim = c(J, N)) + array(error[, nm], dim =
                                                                                        c(J, N))
        PredictM0[, , nm] <- PredictM0b[, , nm] <- array(muM0, dim = c(J, N)) + array(error[, nm], dim =
                                                                                        c(J, N))
        ...
        for (k in 1:length(lmodel.m[[nm]]$lev)) {
          for (j in 1:J) {
            a = which(PredictM1b[j, , nm] > seuil[k] &
                        PredictM1b[j, , nm] <= seuil[k + 1])
            b = which(PredictM0b[j, , nm] > seuil[k] &
                        PredictM0b[j, , nm] <= seuil[k + 1])
        ...
        PredictM1[, , nm] <- array(muM1, dim = c(J, N)) + array(error[, nm], dim =
                                                                  c(J, N))
        PredictM0[, , nm] <- array(muM0, dim = c(J, N)) + array(error[, nm], dim =
                                                                  c(J, N))
    ...
    for (e in 1:4) {
        ...
        if (NM != 1 & e <= 2) {
          for (nm in 1:NM) {
            ...
            for (nmt in 1:NM) {
              ...
              if (inherits(lmodel.m[[nmt]], "lm") &
                  !inherits(lmodel.m[[nmt]], "glm")) {
      ...
      if (!is.null(model.y$family)) {
        ...
        if (model.y$family$link == "logit") {
          ...
          OR[, , e] = ((1 - expit(ORPr0)) / expit(ORPr0)) * ((expit(ORPr1)) /
                                                               (1 - expit(ORPr1)))
        }
        if (NM != 1 & e <= 2) {
          effect.tmp.NM[, , e, ] = (Pr1.NM > 0) - (Pr0.NM > 0)
          if (model.y$family$link == "logit") {
            OR.NM[, , e, ] = ((1 - expit(ORPr0.NM)) / expit(ORPr0.NM)) * ((expit(ORPr1.NM)) /
                                                                            (1 - expit(ORPr1.NM)))
          }
    ...
    if (NM != 1) {
      ...
      if (!is.null(model.y$family)) {
        if (model.y$family$link == "logit") {
          ...
          ORnu.0.NM <- array(ORzeta.1, dim = c(NM, J)) * OReta.0.NM * (ORdelta.0.NM -
                                                                         1) / (array(ORzeta.1 * ORdelta.0, dim = c(NM, J)) - 1)
          ORnu.1.NM <- array(ORzeta.0, dim = c(NM, J)) * OReta.1.NM * (ORdelta.1.NM -
                                                                         1) / (array(ORzeta.0 * ORdelta.1, dim = c(NM, J)) - 1)
          ORdelta.avg.NM <- (ORdelta.1.NM + ORdelta.0.NM) / 2
          ORnu.avg.NM <- (ORnu.1.NM + ORnu.0.NM) / 2
          ORd0.NM <- apply(ORdelta.0.NM * is.finite(ORdelta.0.NM),
                           1,
                           fun,
                           na.rm = TRUE)
          ORd1.NM <- apply(ORdelta.1.NM * is.finite(ORdelta.1.NM),
                           1,
                           fun,
                           na.rm = TRUE)
          ORn0.NM <- apply(ORnu.0.NM * is.finite(ORnu.0.NM), 1, fun, na.rm =
                             TRUE)
          ORn1.NM <- apply(ORnu.1.NM * is.finite(ORnu.1.NM), 1, fun, na.rm =
                             TRUE)
          ...
          logORn0.NM <- apply(logORdelta.0.NM / logORtau.coef, 1, fun, na.rm =
                                TRUE)
          logORn1.NM <- apply(logORdelta.1.NM / logORtau.coef, 1, fun, na.rm =
                                TRUE)
    ...
    if (NM != 1) {
      d0.ci.NM <- d1.ci.NM <- n0.ci.NM <- n1.ci.NM <- d.avg.ci.NM <- n.avg.ci.NM <- array(NA, dim =
                                                                                            c(NM, 2))
      ...
      if (!is.null(model.y$family)) {
        if (model.y$family$link == "logit") {
          ORd0.ci.NM <- ORd1.ci.NM <- ORn0.ci.NM <- ORn1.ci.NM <- ORd.avg.ci.NM <- ORn.avg.ci.NM <- array(NA, dim =
                                                                                                            c(NM, 2))
          ORd0.p.NM <- ORd1.p.NM <- ORd.avg.p.NM <- ORn0.p.NM <- ORn1.p.NM <- ORn.avg.p.NM <- rep(NA, NM)

          logORd0.ci.NM <- logORd1.ci.NM <- logORn0.ci.NM <- logORn1.ci.NM <- logORd.avg.ci.NM <- logORn.avg.ci.NM <- array(NA, dim =
                                                                                                                              c(NM, 2))
          ...
          for (nm in 1:NM) {
            ...
            ORd.avg.p.NM[nm] <- pval(ORdelta.avg.NM[nm, ], ORd.avg.NM[nm], seu =
                                       1)
    ...
    if (NM == 1) {
      ...
      if (!is.null(model.y$family)) {
        if (model.y$family$link == "logit") {
          out <- list(
            ...
            ORn0.sims = ORnu.0,
            ORn1.sims = ORnu.1,
            ORtau.coef = ORtau.coef,
            ORtau.ci = ORtau.ci,
            ORtau.p = ORtau.p,
            ORtau.sims = ORtau,
            ORd.avg = ORd.avg,
            ORd.avg.p = ORd.avg.p,
            ORd.avg.ci = ORd.avg.ci,
            ORd.avg.sims = ORdelta.avg,
            ORz.avg = ORz.avg,
            ORz.avg.p = ORz.avg.p,
            ORz.avg.ci = ORz.avg.ci,
            ORz.avg.sims = ORzeta.avg,
            ORn.avg = ORn.avg,
            ORn.avg.p = ORn.avg.p,
            ORn.avg.ci = ORn.avg.ci,
            ORn.avg.sims = ORnu.avg,
            logORd0 = logORd0,
            logORd1 = logORd1,
            logORd0.ci = logORd0.ci,
            logORd1.ci = logORd1.ci,
            logORd0.p = logORd0.p,
            logORd1.p = logORd1.p,
            logORd0.sims = logORdelta.0,
            logORd1.sims = logORdelta.1,
            logORz0 = logORz0,
            logORz1 = logORz1,
            logORz0.ci = logORz0.ci,
            logORz1.ci = logORz1.ci,
            logORz0.p = logORz0.p,
            logORz1.p = logORz1.p,
            logORz0.sims = logORzeta.0,
            logORz1.sims = logORzeta.1,
            logORn0 = logORn0,
            logORn1 = logORn1,
            logORn0.ci = logORn0.ci,
            logORn1.ci = logORn1.ci,
            logORn0.p = logORn0.p,
            logORn1.p = logORn1.p,
            logORn0.sims = logORnu.0,
            logORn1.sims = logORnu.1,
            logORtau.coef = logORtau.coef,
            logORtau.ci = logORtau.ci,
            logORtau.p = logORtau.p,
            logORtau.sims = logORtau,
            logORd.avg = logORd.avg,
            logORd.avg.p = logORd.avg.p,
            logORd.avg.ci = logORd.avg.ci,
            logORd.avg.sims = logORdelta.avg,
            logORz.avg = logORz.avg,
            logORz.avg.p = logORz.avg.p,
            logORz.avg.ci = logORz.avg.ci,
            logORz.avg.sims = logORzeta.avg,
            logORn.avg = logORn.avg,
            logORn.avg.p = logORn.avg.p,
            logORn.avg.ci = logORn.avg.ci,
            logORn.avg.sims = logORnu.avg
          )
        }
      }
    }
      ...
      out <- list(
        ...
        d0.p.NM = d0.p.NM,
        d1.p.NM = d1.p.NM,
        d0.sims.NM = delta.0.NM,
        d1.sims.NM = delta.1.NM,
        z0 = z0,
        z1 = z1,
        z0.ci = z0.ci,
        z1.ci = z1.ci,
        ...
        d.avg.sims = delta.avg,
        d.avg.NM = d.avg.NM,
        d.avg.p.NM = d.avg.p.NM,
        d.avg.ci.NM = d.avg.ci.NM,
        d.avg.sims.NM = delta.avg.NM,
        z.avg = z.avg,
        z.avg.p = z.avg.p,
        z.avg.ci = z.avg.ci,
        z.avg.sims = zeta.avg,
        ...
        n.avg.sims = nu.avg,
        n.avg.NM = n.avg.NM,
        n.avg.p.NM = n.avg.p.NM,
        n.avg.ci.NM = n.avg.ci.NM,
        n.avg.sims.NM = nu.avg.NM,
        treat = treat,
        mediator = mediator,
        conf.level = conf.level,
        model.y = model.y,
      ...
      if (!is.null(model.y$family)) {
        if (model.y$family$link == "logit") {
          out <- list(
            ...
            ORd0.NM = ORd0.NM,
            ORd1.NM = ORd1.NM,
            ORd0.ci.NM = ORd0.ci.NM,
            ORd1.ci.NM = ORd1.ci.NM,
            ORd0.p.NM = ORd0.p.NM,
            ORd1.p.NM = ORd1.p.NM,
            ORd0.sims.NM = ORdelta.0.NM,
            ORd1.sims.NM = ORdelta.1.NM,
            ORz0 = ORz0,
            ORz1 = ORz1,
            ORz0.ci = ORz0.ci,
            ORz1.ci = ORz1.ci,
            ...
            ORn0.NM = ORn0.NM,
            ORn1.NM = ORn1.NM,
            ORn0.ci.NM = ORn0.ci.NM,
            ORn1.ci.NM = ORn1.ci.NM,
            ORn0.p.NM = ORn0.p.NM,
            ORn1.p.NM = ORn1.p.NM,
            ORn0.sims.NM = ORnu.0.NM,
            ORn1.sims.NM = ORnu.1.NM,
            ORtau.coef = ORtau.coef,
            ORtau.ci = ORtau.ci,
            ORtau.p = ORtau.p,
            ORtau.sims = ORtau,
            ...
            ORd.avg.ci = ORd.avg.ci,
            ORd.avg.sims = ORdelta.avg,
            ORd.avg.NM = ORd.avg.NM,
            ORd.avg.p.NM = ORd.avg.p.NM,
            ORd.avg.ci.NM = ORd.avg.ci.NM,
            ORd.avg.sims.NM = ORdelta.avg.NM,
            ORz.avg = ORz.avg,
            ORz.avg.p = ORz.avg.p,
            ORz.avg.ci = ORz.avg.ci,
            ORz.avg.sims = ORzeta.avg,
            ...
            ORn.avg.ci = ORn.avg.ci,
            ORn.avg.sims = ORnu.avg,
            ORn.avg.NM = ORn.avg.NM,
            ORn.avg.p.NM = ORn.avg.p.NM,
            ORn.avg.ci.NM = ORn.avg.ci.NM,
            ORn.avg.sims.NM = ORnu.avg.NM,
            logORd0 = logORd0,
            logORd1 = logORd1,
            logORd0.ci = logORd0.ci,
            logORd1.ci = logORd1.ci,
            ...
            logORd0.sims = logORdelta.0,
            logORd1.sims = logORdelta.1,
            logORd0.NM = logORd0.NM,
            logORd1.NM = logORd1.NM,
            logORd0.ci.NM = logORd0.ci.NM,
            logORd1.ci.NM = logORd1.ci.NM,
            logORd0.p.NM = logORd0.p.NM,
            logORd1.p.NM = logORd1.p.NM,
            logORd0.sims.NM = logORdelta.0.NM,
            logORd1.sims.NM = logORdelta.1.NM,
            logORz0 = logORz0,
            logORz1 = logORz1,
            logORz0.ci = logORz0.ci,
            logORz1.ci = logORz1.ci,
            ...
            logORn0.sims = logORnu.0,
            logORn1.sims = logORnu.1,
            logORn0.NM = logORn0.NM,
            logORn1.NM = logORn1.NM,
            logORn0.ci.NM = logORn0.ci.NM,
            logORn1.ci.NM = logORn1.ci.NM,
            logORn0.p.NM = logORn0.p.NM,
            logORn1.p.NM = logORn1.p.NM,
            logORn0.sims.NM = logORnu.0.NM,
            logORn1.sims.NM = logORnu.1.NM,
            logORtau.coef = logORtau.coef,
            logORtau.ci = logORtau.ci,
            logORtau.p = logORtau.p,
            logORtau.sims = logORtau,
            ...
            logORd.avg.ci = logORd.avg.ci,
            logORd.avg.sims = logORdelta.avg,
            logORd.avg.NM = logORd.avg.NM,
            logORd.avg.p.NM = logORd.avg.p.NM,
            logORd.avg.ci.NM = logORd.avg.ci.NM,
            logORd.avg.sims.NM = logORdelta.avg.NM,
            logORz.avg = logORz.avg,
            logORz.avg.p = logORz.avg.p,
            logORz.avg.ci = logORz.avg.ci,
            logORz.avg.sims = logORzeta.avg,
            ...
            logORn.avg.ci = logORn.avg.ci,
            logORn.avg.sims = logORnu.avg,
            logORn.avg.NM = logORn.avg.NM,
            logORn.avg.p.NM = logORn.avg.p.NM,
            logORn.avg.ci.NM = logORn.avg.ci.NM,
            logORn.avg.sims.NM = logORnu.avg.NM
          )
        }
      }
    }
